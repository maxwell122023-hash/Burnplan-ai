...
class BurnInputs:
    ...
    simple_smoke_screen_completed: str = ""
    simple_smoke_screen_result: str = ""
    simple_smoke_screen_notes: str = ""
    simple_smoke_screen_attachment_name: str = ""
...
def fill_template(inputs: BurnInputs, weather: WeatherInputs, output_path: str | Path, use_ai: bool = False) -> Path:
    ...
    if "Smoke Screening" in wb.sheetnames:
        del wb["Smoke Screening"]
    smoke_ws = wb.create_sheet("Smoke Screening")
    smoke_ws.append(["Field", "Value"])
    smoke_ws.append(["Simple smoke screening completed", inputs.simple_smoke_screen_completed])
    smoke_ws.append(["Screening result", inputs.simple_smoke_screen_result])
    smoke_ws.append(["Screening notes", inputs.simple_smoke_screen_notes])
    smoke_ws.append(["Attachment filename", inputs.simple_smoke_screen_attachment_name])
    smoke_ws.append(["Nighttime smoke screening", inputs.nighttime_smoke_screening])
    smoke_ws.column_dimensions["A"].width = 35
    smoke_ws.column_dimensions["B"].width = 85
...
def export_pdf(inputs: BurnInputs, weather: WeatherInputs, output_path: str | Path, use_ai: bool = False) -> Path:
    ...
    story.append(section("7. Smoke & Precautions")); story.append(table([["Smoke Sensitive Areas", inputs.smoke_sensitive_areas], ["Nighttime Smoke Screening", inputs.nighttime_smoke_screening], ["Simple Smoke Screening Completed", inputs.simple_smoke_screen_completed], ["Simple Smoke Screening Result", inputs.simple_smoke_screen_result], ["Simple Smoke Screening Notes", inputs.simple_smoke_screen_notes], ["Smoke Screen Attachment", inputs.simple_smoke_screen_attachment_name], ["Special Precautions", inputs.special_precautions], ["Smoke Plan", draft.get("smoke_precautions", inputs.smoke_precautions)]], widths=[2*inch, 4.8*inch]))
