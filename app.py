...
with st.sidebar:
    ...
    st.divider()
    st.write("V3.1.4")
    st.caption("Adds county FWF import, complete project reopening, and Simple Smoke Screening documentation.")
...
with tabs[5]:
    ...
    with ss1:
        simple_smoke_screen_completed = st.selectbox("Simple Smoke Screening Completed", ["", "Yes", "No"])
    with ss2:
        simple_smoke_screen_result = st.selectbox(
            "Simple Smoke Screening Result",
            ["", "No sensitive targets identified", "Targets in secondary zone", "Targets in critical zone"],
        )
    simple_smoke_screen_notes = st.text_area("Simple Smoke Screening Notes", height=90)
    simple_smoke_file = st.file_uploader(
        "Upload Smoke Screening Map or Screenshot",
        type=["pdf", "png", "jpg", "jpeg"],
        key="simple_smoke_file",
    )
    simple_smoke_attachment = None
    if simple_smoke_file is not None:
        simple_smoke_attachment = {
            "filename": simple_smoke_file.name,
            "mime_type": simple_smoke_file.type or "application/octet-stream",
            "data_base64": base64.b64encode(simple_smoke_file.getvalue()).decode("ascii"),
        }
        st.success(f"Attached: {simple_smoke_file.name}")
...
inputs = BurnInputs(
    ...
    simple_smoke_screen_completed=simple_smoke_screen_completed, simple_smoke_screen_result=simple_smoke_screen_result,
    simple_smoke_screen_notes=simple_smoke_screen_notes, simple_smoke_screen_attachment_name=(simple_smoke_attachment or {}).get("filename", ""),
...
with tabs[10]:
    ...
    complete_payload = {
        ...
        "attachments": {"simple_smoke_screen": simple_smoke_attachment} if simple_smoke_attachment else {},
    }
    ...
    def _load_editor_state(payload: dict) -> None:
        ...
        st.session_state["edit_simple_smoke_attachment"] = payload.get("attachments", {}).get("simple_smoke_screen")
    ...
    if st.session_state.get("editor_project_loaded"):
        ...
        with edit_tabs[4]:
            ...
            with a:
                ...
                result_options = ["", "No sensitive targets identified", "Targets in secondary zone", "Targets in critical zone"]
                result_current = st.session_state.get("edit_simple_smoke_screen_result", "")
                st.selectbox("Simple Smoke Screening Result", result_options, index=result_options.index(result_current) if result_current in result_options else 0, key="edit_simple_smoke_screen_result")
                st.text_area("Simple Smoke Screening Notes", key="edit_simple_smoke_screen_notes", height=90)
                existing_attachment = st.session_state.get("edit_simple_smoke_attachment")
                if existing_attachment:
                    st.info(f"Current attachment: {existing_attachment.get('filename', 'Smoke screen attachment')}")
                replacement = st.file_uploader("Replace Smoke Screening Map or Screenshot", type=["pdf", "png", "jpg", "jpeg"], key="edit_simple_smoke_upload")
                if replacement is not None:
                    st.session_state["edit_simple_smoke_attachment"] = {
                        "filename": replacement.name,
                        "mime_type": replacement.type or "application/octet-stream",
                        "data_base64": base64.b64encode(replacement.getvalue()).decode("ascii"),
                    }
                    st.session_state["edit_simple_smoke_screen_attachment_name"] = replacement.name
                    st.success(f"Replacement attached: {replacement.name}")
        ...
        with edit_tabs[7]:
            ...
            updated_project = {
                ...
                "attachments": {"simple_smoke_screen": st.session_state.get("edit_simple_smoke_attachment")} if st.session_state.get("edit_simple_smoke_attachment") else {},
            }
